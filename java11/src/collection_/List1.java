...
public class List1 {
	public static void main(String[] args) {
		...
		// 그 중 List 라는 클래스가 있음 (배열의 어려운점을 자동화해주는 클래스)
		// List를 상속해서 ArrayList라는 클래스를 자바에서 제공해주고 ArrayList로 주로 사용됨
		
		// ArrayList는 다른 패키지에 있기 때문에 util.ArrayList를 import
		// new ArrayList<생략이 가능(String)>();
		ArrayList<String> arr1 = new ArrayList<>();		// String 배열
		ArrayList<Integer> arr2 = new ArrayList<>();	// int(Integer) 배열 // Integer == Java 구조상 한계
		ArrayList<Double> arr3 = new ArrayList<>();		// double 배열 (가변 배열)
		...
		for(int i=0; i<arr2.size(); i++) {
			System.out.println(arr2.get(i));	// 배열의 경우 값을 넣지않은 공간에 대해서는  if문 처리
		}
	}
}
